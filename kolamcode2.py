...
def draw_tile_clean(dwg, g_art, g_decode, x, y, s, rot, dotbit,
                    stroke_decode="black", decode_w=1.7):
    """Draw clean style: only essential lines and dots - nothing extra."""
    half = s/2.0
    cx, cy = x+half, y+half
    
    # Essential connection points - no offsets for perfect alignment
    mids = [(x, cy), (cx, y), (x + s, cy), (cx, y + s)]
    ...
    # Single clean flowing line - only essential curves
    def clean_curve(p0, p1):
        mid_x, mid_y = (p0[0] + p1[0])/2, (p0[1] + p1[1])/2
        
        # Minimal control points for smooth flow
    ...
    # Only the two essential curves
    clean_curve(L, T)
    clean_curve(R, B)

    # Essential decode layer - thin arcs for QR functionality
    r = 0.95*half
    def arc_cmd(a1, a2):
        def pt(a):
            rad = a*pi/180.0
    ...
    # Essential rotation mapping
    arcs = [(180,270),(0,90)] if rot == 0 else \
           [(270,360),(90,180)] if rot == 1 else \
           [(0,90),(180,270)] if rot == 2 else \
           [(90,180),(270,360)]
    
    for a1,a2 in arcs:
        g_decode.add(dwg.path(d=arc_cmd(a1,a2), fill="none", stroke=stroke_decode,
                             stroke_width=decode_w, stroke_linecap="round"))
    
    # Only add dot if it's actually needed
    if dotbit == 1:
        # Essential decode dot
        g_decode.add(dwg.circle(center=(cx,cy), r=s*0.06, fill=stroke_decode))
        # Essential art dot - simple and minimal
...
def render_svg_clean(grid_bits: List[int], N: int, tile_bits=3,
                     out_svg="kolam.svg", tile_px=42, margin_px=180, stroke_px=3.0,
                     dot_opacity=None):
    """Clean renderer: only essential elements - lines and dots that are actually used."""
    total_tiles = N*N
    ...
    # Only the layers that are actually used
    g_art = dwg.add(dwg.g())       # kolam lines
    g_decode = dwg.add(dwg.g())    # thin arcs for QR decoding

    # Check if any tiles actually have dots before creating dot lattice
    has_dots = any(bits[i*3+2] == 1 for i in range(total_tiles))
    
    if has_dots:
        # Only add dot lattice if there are actual dots in the pattern
    ...
    # tiles - only draw what's needed
    k = 0
    rots = []  # Store rotations for connector pass
    for r in range(N):
        for c in range(N):
            ...
            x = margin_px + c*tile_px
            y = margin_px + r*tile_px
            draw_tile_clean(dwg, g_art, g_decode, x, y, tile_px, rot, b2)

    # Convert to 2D array for neighbor checking
    rots_2d = np.array(rots).reshape((N,N))

    # Optimized connector drawing - only draw where actually needed
    def draw_clean_connector(x, y, side):
        s = tile_px
        half = s/2.0
        cx, cy = x+half, y+half
    ...
    # Only draw connectors where tiles actually connect
    for r in range(N):
        for c in range(N-1):
            rotA = rots_2d[r,c]; rotB = rots_2d[r,c+1]
            edgesA = _edges_for_rot(rotA)
            edgesB = _edges_for_rot(rotB)
            if any(2 in p for p in edgesA) and any(0 in p for p in edgesB):
                x = margin_px + c*tile_px
                y = margin_px + r*tile_px
                draw_clean_connector(x, y, side=2)

    for r in range(N-1):
        for c in range(N):
            rotA = rots_2d[r,c]; rotB = rots_2d[r+1,c]
            edgesA = _edges_for_rot(rotA)
            edgesB = _edges_for_rot(rotB)
            if any(3 in p for p in edgesA) and any(1 in p for p in edgesB):
                x = margin_px + c*tile_px
                y = margin_px + r*tile_px
                draw_clean_connector(x, y, side=3)
